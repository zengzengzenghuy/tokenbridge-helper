...
contract Erc20ToNativeBridgeHelper is Helper {
    ...
    function getMessageHash(address _recipient, uint256 _value, bytes32 _nonce) external view returns (bytes32) {
        bytes32 result = keccak256(abi.encodePacked(_recipient, _value, _nonce, foreignBridge));
        return result;
    }

    function getHashiMessageHash(address _recipient, uint256 _value, bytes32 _nonce) external view returns (bytes32){
        bytes32 result =  keccak256(abi.encodePacked(_recipient, _value, _nonce));
        return result;
    }
